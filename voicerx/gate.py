...
# The register in BENGALI. It holds 179k brand names in Latin only, and the
# ASR emits Bengali - so for a spoken brand the biggest table in the system
# was unreachable. Generated by rule from those same entries (see
# voicerx/translit.py) and audited on generation against the curated tables,
# so nothing here can shadow a reviewed entry.
#
# It joins the register, NOT the curated gazetteer, and therefore inherits
# the register's rule: it may VALIDATE a name the model asserted, and is
# never scanned out of free text. Machine-generated keys get the weakest
# standing in the file, which is the correct place for them.
try:
    from .brands_bengali import BENGALI_BRANDS
except Exception:                                      # noqa: BLE001
    BENGALI_BRANDS = {}
for _bb, _bg in BENGALI_BRANDS.items():
    _k = fold(_bb)
    if len(_k) >= 6:
        _BRAND_LOOKUP.setdefault(_k, _bg)
